...
function cdf( x, options ) {
	...
	if ( opts.b <= opts.a ) {
		throw new TypeError( 'cdf()::invalid input options. Parameter `a` must be smaller than `b`.' );
	}

	if ( isNumber( x ) ) {
		return cdf1( x, opts.a, opts.b );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return cdf5( out, x, opts.a, opts.b );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.a, opts.b );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.a, opts.b, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.a, opts.b, opts.accessor );
		}
		return cdf2( out, x, opts.a, opts.b );
	}
	return NaN;
} // end FUNCTION cdf()
